...
from .const import (
    ...
    GOOGLE_SHEETS_MAX_EVENT_DURATION,
...
def _event_frame(event: dict) -> int | None:
    ...
    for key in ("frame", "from"):
        if payload.get(key) is None:
            continue
        # "from" is only a fallback for change:frame when frame was not stamped
        if key == "from" and event.get("scope") != "change:frame":
            continue
...
def _read_ids(event: dict) -> tuple[int | None, int | None, int | None, int | None]:
    return event.get("job_id"), event.get("task_id"), event.get("project_id"), _event_frame(event)


def _max_event_duration_for_data(data: dict) -> datetime.timedelta:
    candidates = list(data.get("events") or [])
    if previous_event := data.get("previous_event"):
        candidates.append(previous_event)
    if any(event.get("source") == GOOGLE_SHEETS_SOURCE for event in candidates):
        return GOOGLE_SHEETS_MAX_EVENT_DURATION
    return MAX_EVENT_DURATION


def compute_working_time_per_ids(data: dict) -> dict:
    """
    Sum working time per (job, task, project, frame).

    Gaps shorter than the source-specific max duration count as working time
    (100s for client, 600s for google_sheets).
    """
    events = data["events"]
    if not events:
        return {}

    max_event_duration = _max_event_duration_for_data(data)

    if previous_event := data["previous_event"]:
        previous_end_timestamp = get_end_timestamp(previous_event)
        previous_ids = _read_ids(previous_event)
    else:
        previous_end_timestamp = events[0]["timestamp"]
        previous_ids = _read_ids(events[0])

    working_time_per_ids: dict = {}
    for event in events:
        ...
        if timestamp > working_time_per_ids[previous_ids]["timestamp"]:
            working_time_per_ids[previous_ids]["timestamp"] = timestamp
        previous_ids = _read_ids(event)
...
def compute_interactor_response_time_per_ids(data: dict) -> dict:
    """
    Sum client-reported interactor wall-clock waits per (job, task, project, frame).

    Includes ok / error / cancelled — all are non-active wait time for the annotator.
    """
    interactor_ms_per_ids: dict[tuple, int] = {}
    for event in data.get("events") or []:
        if event.get("scope") != INTERACTOR_RESPONSE_SCOPE:
            continue
        payload = _parse_payload(event)
        raw = payload.get("interactor_response_time", event.get("duration", 0))
        try:
            ms = int(raw)
        ...
        if ms <= 0:
            continue
        ids = _read_ids(event)
        interactor_ms_per_ids[ids] = interactor_ms_per_ids.get(ids, 0) + ms
    return interactor_ms_per_ids
