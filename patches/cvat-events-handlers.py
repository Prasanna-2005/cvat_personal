...
from .const import GOOGLE_SHEETS_SOURCE, WORKING_TIME_RESOLUTION, WORKING_TIME_SCOPE
...
def handle_client_events_push(request, data: dict):
    ...
    if data["events"]:
        # Serializer stamps canonical identity onto each event (request.user for
        # client; delegated annotator for google_sheets). Prefer event fields so
        # send:working_time follows the same attribution.
        first_event = data["events"][0]
        common = {
            "user_id": first_event.get("user_id", request.user.id),
            "user_name": first_event.get("user_name", request.user.username),
            "user_email": first_event.get("user_email") or None,
            "org_id": first_event.get("org_id", getattr(org, "id", None)),
            "org_slug": first_event.get("org_slug", getattr(org, "slug", None)),
        }
        from_google_sheets = any(
            event.get("source") == GOOGLE_SHEETS_SOURCE for event in data["events"]
        )
        ...
        if job_ids:
            for jid, stage, tid, pid in Job.objects.filter(id__in=job_ids).values_list(
                "id", "stage", "segment__task_id", "segment__task__project_id"
            ):
                job_meta[jid] = {"stage": stage, "task_id": tid, "project_id": pid}

        for ids in all_ids:
            job_id, task_id, project_id, frame = ids
            working_time = working_time_per_ids.get(ids)
            working_ms = (
            ...
            interactor_ms = interactor_ms_per_ids.get(ids, 0)

            if not working_ms and not interactor_ms:
                continue

            # keep working_time in payload for backward compatibility
            # but in the future it is much better to use a "duration" field
            # because parsing JSON in SQL query is very slow
            payload: dict[str, Any] = {"working_time": working_ms}
            ...
            if from_google_sheets:
                payload["origin"] = GOOGLE_SHEETS_SOURCE
